...
async function importLibraryBundle(input) {
  const file = input.files[0]; if (!file) return; input.value = '';
  let bundle;
  try { bundle = JSON.parse(await file.text()); } catch { alert('Invalid JSON file.'); return; }
  if (bundle.format !== 'schematic-library-v1') {
    alert('Unrecognised format. Expected a library.json exported from Schematic Designer.'); return;
  }
  ...
  const ticketNote = d.ticketsImported ? `\n${d.ticketsImported} generate ticket(s) imported.` : '';
  const msg = d.installed.length
    ? `Installed ${d.installed.length} new component(s): ${d.installed.join(', ')}.${d.skipped.length ? `\n${d.skipped.length} already present — skipped.` : ''}${ticketNote}`
    : `All ${d.skipped.length} component(s) already in your library — nothing to install.${ticketNote}`;
  alert(msg);
